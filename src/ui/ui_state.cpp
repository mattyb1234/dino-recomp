...
public:
    ...
    UIState(SDL_Window* window, plume::RenderInterface* interface, plume::RenderDevice* device) {
        ...
        Rml::Debugger::Initialise(context);
        {
            ...
            for (const FontFace& face : font_faces) {
                auto font = dino::runtime::get_asset_path(face.filename);
    ...
    void create_menus() {
        recompui::init_styling(dino::runtime::get_asset_path("recomp.rcss"));
...
int cont_button_to_key(SDL_ControllerButtonEvent& button) {
    ...
    auto menuAcceptBinding0 = dino::input::get_input_binding(dino::input::GameInput::ACCEPT_MENU, 0, dino::input::InputDevice::Controller);
    auto menuAcceptBinding1 = dino::input::get_input_binding(dino::input::GameInput::ACCEPT_MENU, 1, dino::input::InputDevice::Controller);
    ...
    auto menuApplyBinding0 = dino::input::get_input_binding(dino::input::GameInput::APPLY_MENU, 0, dino::input::InputDevice::Controller);
    auto menuApplyBinding1 = dino::input::get_input_binding(dino::input::GameInput::APPLY_MENU, 1, dino::input::InputDevice::Controller);
    // note - magic number: 0 is InputType::None
...
void apply_background_input_mode() {
    static dino::input::BackgroundInputMode last_input_mode = dino::input::BackgroundInputMode::OptionCount;

    dino::input::BackgroundInputMode cur_input_mode = dino::input::get_background_input_mode();

    if (last_input_mode != cur_input_mode) {
        SDL_SetHint(
            SDL_HINT_JOYSTICK_ALLOW_BACKGROUND_EVENTS,
            cur_input_mode == dino::input::BackgroundInputMode::On
                ? "1"
                : "0"
        );
    }
...
void draw_hook(plume::RenderCommandList* command_list, plume::RenderFramebuffer* swap_chain_framebuffer) {
    ...
    dino::input::config_menu_set_cont_or_kb(ui_state->cont_is_active);

    dino::input::InputField scanned_field = dino::input::get_scanned_input();
    if (scanned_field != dino::input::InputField{}) {
        dino::input::finish_scanning_input(scanned_field);
    }
